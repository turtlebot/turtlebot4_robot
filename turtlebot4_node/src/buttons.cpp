...
Buttons::Buttons(Turtlebot4Model model,
                 std::vector<Turtlebot4Button> buttons,
                 std::shared_ptr<rclcpp::Node> &nh, 
                 std::shared_ptr<GpioInterface> gpio_interface, bool use_sim) : user_button_pins_{HMI_BUTTON_1_PIN,
                                                                                                  HMI_BUTTON_2_PIN, 
                                                                                                  HMI_BUTTON_3_PIN,
                                                                                                  HMI_BUTTON_4_PIN},
                                                                                buttons_(buttons),
                                                                                ...
                                                                                model_(model)
{
    ...
    {
        hmi_buttons_pub_ = nh_->create_publisher<turtlebot4_msgs::msg::UserButton>("hmi/buttons", rclcpp::QoS(rclcpp::KeepLast(10)));
        if (use_sim)
        {
...
void Buttons::create3_buttons_callback(const irobot_create_msgs::msg::InterfaceButtons::SharedPtr create3_buttons_msg)
{
    buttons_.at(CREATE3_1).set_state(static_cast<Turtlebot4ButtonState>(create3_buttons_msg->button_1.is_pressed));
    buttons_.at(CREATE3_POWER).set_state(static_cast<Turtlebot4ButtonState>(create3_buttons_msg->button_power.is_pressed));
    buttons_.at(CREATE3_2).set_state(static_cast<Turtlebot4ButtonState>(create3_buttons_msg->button_2.is_pressed));
}
