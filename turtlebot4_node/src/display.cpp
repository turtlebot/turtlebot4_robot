 ...
 */
Display::Display(std::vector<Turtlebot4MenuEntry> entries,
                 std::shared_ptr<rclcpp::Node> &nh, std::shared_ptr<I2cInterface> i2c, std::shared_ptr<GpioInterface> gpio, bool use_sim) : nh_(nh),
                                                                    ...
                                                                    use_sim_(use_sim)
{
    ...
    display_raw_pub_ = nh_->create_publisher<std_msgs::msg::String>("hmi/display/_raw", rclcpp::QoS(rclcpp::KeepLast(10)));
    display_selected_pub_ = nh_->create_publisher<std_msgs::msg::Int32>("hmi/display/_selected", rclcpp::QoS(rclcpp::KeepLast(10)));
...
void Display::draw_header()
{
    ...
    oled_.Line(0, SSD1306_HEADER_FONT_HEIGHT + 1, SSD1306_WIDTH, SSD1306_HEADER_FONT_HEIGHT + 1, White);
}
...
void Display::show_message(std::string message)
{
    ...
    for (int i = 0; i < SSD1306_NUM_LINES; i++)
    {
        ...
        {
            display_lines_[i] = message.substr(SSD1306_CHAR_PER_LINE * i, message.length() - (SSD1306_CHAR_PER_LINE * i));
        }
        else
        {
    ...
    if (!use_sim_)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < visible_entries_.size(); i++)
            {
                ...
                if (i == selected_line_)
                {
                    oled_.Line(0, oled_.GetCursor().CurrentY - 1, visible_entries_.at(i).name_.length() * SSD1306_FONT_WIDTH - 1, oled_.GetCursor().CurrentY - 1, White);
