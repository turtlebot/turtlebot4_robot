...
namespace turtlebot4
{
    ...
    // Constructor and Destructor
        explicit Turtlebot4Service(std::shared_ptr<rclcpp::Node> &nh, std::string service) : nh_(nh), service_(service)
        {
        ...
        void make_request(std::shared_ptr<typename ServiceT::Request> request)
        {
            ...
            timer_ = nh_->create_wall_timer(
                ...
                [this]() -> void
                {
                    ...
                    {
                        RCLCPP_ERROR(nh_->get_logger(), "Interrupted while waiting for the service %s.", service_.c_str());
                        timer_->cancel();
                    }
                    else
                    {
    ...
    private:
        ...
        // TODO: Virtual callback?
        // Service response callback
        void response_callback(typename rclcpp::Client<ServiceT>::SharedFuture future)
        {
            auto result = future.get();
            RCLCPP_INFO(nh_->get_logger(), "%s service got results: %s", service_.c_str(), result->success ? "Success" : "Failed");
