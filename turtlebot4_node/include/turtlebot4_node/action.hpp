...
namespace turtlebot4
{
    ...
    public:
        ...
        explicit Turtlebot4Action(std::shared_ptr<rclcpp::Node> &nh, std::string action) : nh_(nh), action_(action)
        {
        ...
        void send_goal()
        {
            ...
            timer_ = nh_->create_wall_timer(
                ...
                [this]() -> void
                {
                    ...
                    {
                        RCLCPP_INFO(nh_->get_logger(), "%s action server available, sending goal", action_.c_str());
                        timer_->cancel();
                        ...
                        send_goal_options.feedback_callback =
                            std::bind(&Turtlebot4Action::feedback_callback, this, std::placeholders::_1, std::placeholders::_2);
    ...
    private:
        ...
        void goal_response_callback(std::shared_future<typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr> future)
        {
            ...
            {
                RCLCPP_INFO(nh_->get_logger(), "%s goal accepted by server, waiting for result", action_.c_str());
            }
        }
        ...
        /**
          ...
          */
        void feedback_callback(typename rclcpp_action::ClientGoalHandle<ActionT>::SharedPtr ptr, const std::shared_ptr<const typename ActionT::Feedback> feedback)
        {
            (void)ptr;
            (void)feedback;
            //RCLCPP_INFO(nh_->get_logger(), "Feedback");
        }
